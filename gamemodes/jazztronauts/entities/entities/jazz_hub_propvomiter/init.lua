...
local stuckchance = GetConVar("jazz_propvomiter_stuckchance"):GetInt()
local stuckvomitamount = GetConVar("jazz_propvomiter_stuckvomitamount"):GetInt()
local stuckminamount = GetConVar("jazz_propvomiter_stuckminamount"):GetInt()

ENT.VomitVelocity = Vector(0, 0, -200)
ENT.MaxPipeSize = 50
ENT.ConstipateOdds = stuckchance -- The odds to do the constipated thing (0 is always, 50 is 1/50)
ENT.ConstipateCount = stuckvomitamount -- How many props to spawn at once
ENT.ConstipateMinProps = stuckminamount -- Minimum number of props required to roll the dice for constipation
...
function ENT:Think()
	...
	if not self.Constipated then
		local vomitspeed = GetConVar("jazz_propvomiter_speed"):GetInt()
		self:VomitMultiple(vomitspeed)
	end
